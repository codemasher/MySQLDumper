...
} // language not changed, go on as usual
...
if(isset($_GET['config'])){
	...
	if(read_config($_GET['config'])){
		...
		$msg                     .= '<script>parent.MySQL_Dumper_menu.location.href="menu.php?config='.$config['config_file'].'";</script>';
	}
	else{
...
if(isset($_GET['config_delete'])){
	...
	if($del === false){
		...
		$msg = '<p class="success">'.sprintf($lang['L_SUCCESS_DELETING_CONFIGFILE'], $del_config).'</p>'.'<script>parent.MySQL_Dumper_menu.location.href="menu.php?config='.$config['config_file'].'";</script>';
	} //refresh menu-frame
	$sel = 'configs';
}
...
if(isset($_POST['load'])){
	...
	echo '<script>parent.MySQL_Dumper_menu.location.href="menu.php";</script>';
}
...
if(isset($_POST['save'])){
	...
	if($config['multipartgroesse1'] < 100 && $config['multipartgroesse2'] == 1){
	...
	if($config['dbhost'] != $_POST['dbhost'] || $config['dbuser'] != $_POST['dbuser'] || $config['dbpass'] != $_POST['dbpass'] || $config['dbport'] != $_POST['dbport'] || $config['dbsocket'] != $_POST['dbsocket']){
		...
		if(MSD_mysql_connect()){
			...
			SetDefault();
			$msg .= '<script>parent.MySQL_Dumper_menu.location.href="menu.php";</script>';
		}
		else{
			//alte Werte holen
	...
	if(sizeof($databases['praefix']) > $anzahl_datenbanken){
		...
		if($databases['db_selected_index'] >= $anzahl_datenbanken){
	...
	if($save_config){
		if(WriteParams(false) == true){
			...
			if($_SESSION['config']['language'] != $config['language'] || $_POST['scaption_old'] != $config['interface_server_caption'] || $oldtheme != $config['theme'] || $oldscposition != $config['interface_server_caption_position']){
				$msg .= '<script>parent.MySQL_Dumper_menu.location.href="menu.php?config='.urlencode($config['config_file']).'";</script>';
...
?>
	<script>
		...
		function WriteMem(){
			...
			echo round((int)$config['php_ram'] * 1024 * 1024 * 0.9, 0);
			?>;
		}
	</script>
<?php
...
if(($showVP) || (!isset($databases['Name'])) || (isset($databases['name']) && count($databases['Name'] == 0)) || (isset($add_db_message))){
	echo 'SwitchVP();';
}
echo '</script>';
echo MSDFooter();
