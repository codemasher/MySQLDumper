<div id="content">
	<h3>Sur ce projet</h3>
	L'idée pour ce projet est venue de Daniel Schlichtholz.<p>Il a ouvert en 2004 le forum
		<a href="http://forum.mysqldumper.de" target="_blank">MySQLDumper</a> et rapidement d'autres développeurs ont écrit de nouveaux scripts ou bien élargi les scripts de Daniel. En peu de temps le simple script de sauvegarde est devenu un imposant projet.
	<p>Si vous avez des propositions d'améliorations vous pouvez les communiquer dans le Forum MySQLDumper
		<a href="http://forum.mysqldumper.de" target="_blank">http://forum.mysqldumper.de</a>.
	<p>Nous vous souhaitons beaucoup de plaisir avec ce projet.<br>
	<p><h4>L'équipe de MySQLDumper</h4>
	...
	<ul>
		...
		<ul><br>
			<li>
				<a name="conf1"></a><strong>Configurer la base de données:</strong> Liste des bases de données configurées. La base de données active est listée en
				<b>caractères gras</b>.
			</li>
			<li>
				<a name="conf2"></a><strong>Préfix des tables:</strong> Ici vous pouvez définir (pour chaque base de donnée) un préfix. C'est un filtre, qui permet de sélectionner lors de la sauvegarde les tables contenant le préfix défini (exemple: Toutes les tables qui commencent avec "phpBB_"). Si vous désirer sauvgarder toutes les tables, laisser ce champ libre.
			</li>
			<li>
				<a name="conf3"></a><strong>Compression GZip:</strong> Ici vous pouvez activer la compression de fichier. Nous vous conseillons la compression, car les fichiers sont plus petit et nécessitent moins de place.
			</li>
			<li>
				<a name="conf5"></a><strong>Courriel avec pièces jointes:</strong> Si vous avez activé cette option, vous recevrez après la création de la copie de sauvegarde un courriel avec la copie de sauvegarde en pièce jointe (Attention, la compression doit être impérativement activée, sinon la pièce jointe risque d'être trop volumineuse et ne pourra eventuellement pas être envoyée avec le courriel!)
			</li>
			<li><a name="conf6"></a><strong>Adresse électronique:</strong> Destinataire du courriel</li>
			<li>
				<a name="conf7"></a><strong>Expéditeur du courriel:</strong> C'est l'adresse de l'expéditeur qui apparaitra dans le courriel
			</li>
			<li>
				<a name="conf13"></a><strong>Transfert FTP: </strong>Si cette option est activée, après la création de la copie de sauvegarde, celle-ci sera envoyée par FTP sur un serveur.
			</li>
			...
			<li>
				<a name="conf18"></a><strong>Répertoire de téléchargement FTP: </strong>Le répertoire dans lequel la copie de sauvegarde doit être copiée (il doit exister des droits afin de pouvoir télécharger vers le serveur!)
			</li>
			<li>
				<a name="conf8"></a><strong>Suppression automatique de la copie de sauvegarde:</strong> Si cette option est activée, les sauvegardes les plus anciennes seront supprimée d'après les options choisies. La combinaison d'options n'est pas possible.
			</li>
			<li>
				<a name="conf10"></a><strong>Nombre de copie de sauvegarde:</strong> La valeur > 0 supprime toutes les copies de sauvegardes les plus anciennes excepté le nombre défini dans ce champ
			</li>
		...
		<UL>
			...
			<li>
				<strong>Exécuter une nouvelle copie de sauvegarde:</strong> Exécution d'une nouvelle copie de sauvegarde (Dump) d'après les paramètres du menu "Configuration".
			</li>
		</UL>
