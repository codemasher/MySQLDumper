...
<form action="sql.php{TARGET}" method="post">
	...
	<table class="bdr">
		<tr class="thead">
			<th colspan="3">{L_SQL_RECORDEDIT}</th>
		</tr>
		<tr class="thead">
			<th>{L_NAME}</th>
			<th>NULL</th>
		...
		<tr class="dbrow{ROW.CLASS}">
			...
			<td nowrap="nowrap">&nbsp;
				<!-- BEGIN IS_NULLABLE -->
				<input type="checkbox" name="null_{ROW.FIELD_ID}" id="null_{ROW.FIELD_ID}" onchange="switch_area('{ROW.FIELD_ID}')"{ROW.IS_NULLABLE.NULL_CHECKED}>
				&nbsp;
