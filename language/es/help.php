<div id="content">
	...
	<p>En 2004 abrió el foro
		<a href="http://forum.mysqldumper.de" target="_blank">MySQLDumper</a> e inmediatamente algunos aficionados a la programación se encontraron allí, para escribir y ampliar la versión inicial de los scripts de Daniel.<br>
		En poco tiempo quedó establecido el proyecto de realizar un script de copia de seguridad.
	<p>Si tiene Vd. alguna propuesta de mejora, notifíquela en el foro de MySQLDumper-Forum:
		<a href="http://forum.mysqldumper.de" target="_blank">http://forum.mysqldumper.de</a>.
	...
	<table>
		<tr>
			<td><img src="images/logo.gif" alt="MySQLDumper" border="0"></td>
			<td valign="top">
	...
	<p>En la mayoría de los casos, el servidor dispondrá de un directorio llamado cgi-bin (o perl) desde el que pueden ejecutarse scripts perl.
		<br>
		...
		<b>Nota:</b> Tanto los usuarios de Windows como los usuarios de servidores con configuraciones no estándar, deberán cambiar en los tres scripts la primera línea para reflejar el camino correcto de Perl. Por ejemplo:
		<br>
	...
	<ul>
		...
		<ul>
			<li>
				<b><?php echo $lang['Statusinformationen'] ?></b> le mostrará las informaciones genéricas, pudiendo además acceder a algunas de ellas en particular, para ampliarlas.
			</li>
			<li>
				<b><?php echo $lang['dbs'] ?></b> le llevará a la lista de las mismas, pudiendo crear otras nuevas. Si hace click en alguna de ellas, se le llevará a un menú avanzado dónde se le mostrarán las tablas que contiene y distintas opciones para con ellas.
			</li>
			<li>
				<b><?php echo $lang['mysqlvars'] ?></b> le mostrará respectivamente los procesos, el estado y las opciones y definiciones del servidor de base de datos MySQL.
			</li>
			<li>
				<b><?php echo $lang['mysqlsys'] ?></b> le permitirá acceder a una pseudoconsola del servidor de bases de datos y realizar operaciones complejas con el mismo. Nota: para poder utilizar dichas opciones, deberá tener los privilegios adecuados en el servidor MySQL.
			</li>
		</ul>
		...
		<ul>
			<li>
				<b><?php echo $lang['dbs'] ?></b> le permite cambiar los parámetros de conexión (haga click en mostrar / esconder) del usuario de base de datos. Si hay más de una base de datos, puede elegir hacer un volcado múltiple e incluir más de una bases de datos en la copia de seguridad (la base de datos actual se muestra siempre en
				<b>negrita</b>). Además, podrá seleccionar las tablas que serán incluidas en la copia de seguridad mediante un prefijo.
			</li>
			<ul>
				<li>
					<a name="conf1"></a><b><?php echo $lang['help_db'] ?></b> muestra un listado de todas las bases de datos accesibles. Si ha especificado en los parámetros de conexión, que solamente se muestre un tipo una base de datos, solamente aparecerá ésta. Si hay más de una base de datos accesible, puede elegir hacer un volcado múltiple e incluir más de una bases de datos en la copia de seguridad (la base de datos actual se muestra siempre en
					<b>negrita</b>). Además, podrá seleccionar las tablas de cada base de datos que deben ser incluidas en la copia de seguridad mediante un prefijo, excluyendo las que no contengan el mismo.
				</li>
				<li>
					<a name="conf2"></a><?php echo $lang['help_praefix'] ?> es el prefijo que puede especificar para seleccionar tablas de una base de datos. Por ejemplo puede especificar solamente aquellas tablas que empiecen con el prefijo "phpBB_". Si desea hacer una copia de seguridad de toda la base de datos, deje este campo en blanco.
				</li>
			</ul>
			<li>
				<b><?php echo $lang['general'] ?></b> sirve para elegir las características genéricas de las copias de seguridad (compresión, memoria, velocidad [ALERTA: la velocidad excesiva puede provocar que el servidor deje de responder (timeout)], archivos de registro o logs, si se deben optimizar las tablas antes de hacer la copia, etc...) y de la restauración de datos (si se deben vaciar las tablas antes de hacerla, si se debe detener la importación en caso de errores).
			</li>
			<ul>
				<li>
					<a name="conf3"></a><b><?php echo $lang['gzip'] ?></b> permite activar la compresión de los archivos. Se recomienda activarla, si el módulo GZIP está disponible en su servidor, ya que el tamaño de los archivos se reduce sensiblemente.
				</li>
			...
			<li>
				<b><?php echo $lang['config_interface'] ?></b> permite elegir las características gráficas de la interfaz del programa. Puede elegir idioma, tema, definir algunos tamaños de ventana, incluso decidir si desea que aparezca el nombre del servidor en que se encuentra en este momento, y en qué lugar. La elección del navegador que utiliza es importante, ya que si lo hace de forma incorrecta, el programa no funcionará correctamente.
			</li>
			<ul>
				<li>
					<a name="conf11"></a><b><?php echo $lang['help_lang'] ?>:</b> aquí puede seleccionar el idioma para el interfaz gráfico.
				</li>
			</ul>
			<li>
				<b><?php echo $lang['config_autodelete'] ?></b> define los parámetros que determinan si se van a eliminar archivos de copia de seguridad de forma automática o no.
			</li>
			<ul>
				<li>
					<a name="conf8"></a><b><?php echo $lang['help_ad1'] ?>:</b> activa o desactiva la eliminación augomática. Si está activado, se eliminarán los archivos necesarios (según las reglas definidas a continuación) antes de iniciar una nueva copia de seguridad. Es una opción útil para ahorrar espacio en el servidor, pero le recomendamos que no la active antes de haber podido probar el funcionamiento correcto del programa.
				</li>
				<li>
					<a name="conf10"></a><b><?php echo $lang['help_ad3'] ?>:</b> un valor mayor que cero elimina todos los archivos de copia de seguridad en exceso del número especificado, bien en total, bien para cada base de datos distinta.
				</li>
			</ul>
			<li>
				<b><?php echo $lang['config_email'] ?></b> define los parámetros que determinan si se va a enviar un email tras haber completado una copia de seguridad, así como si se deberá adjuntar dicha copia de seguridad y en qué forma.
			</li>
			<ul>
				<li>
					<a name="conf8"></a><b><?php echo $lang['help_mail1'] ?>:</b> activa o desactiva el envío de un email al haberse terminado la copia de seguridad, sea con o sin éxito.
				</li>
				<li>
					<a name="conf9"></a><b><?php echo $lang['help_mail2'] ?>:</b> es la dirección de email a dónde se enviará el mensaje.
				</li>
				<li>
					<a name="conf10"></a><b><?php echo $lang['help_mail3'] ?>:</b> es la dirección de email desde donde se enviará el mensaje. Recuerde permitirle el paso a través de su filtro anti-spam, si dispone de uno.
				</li>
			</ul>
			<li>
				<b><?php echo $lang['config_ftp'] ?></b> permite definir una (o varias) transferencias por FTP del archivo de copia de seguridad una vez terminada la misma. Si se activa, se deben especificar las parámetros necesarios para realizar la conexión. Además, deberá tener los derechos apropiados en el servidor de destino de la copia.
			</li>
			<ul>
				<li>
					<a name="conf13"></a><b><?php echo $lang['help_ftptransfer'] ?>:</b> activa o desactiva el envío de la copia de seguridad realizada con éxito, por FTP.
				</li>
				<li>
					<a name="conf14"></a><b><?php echo $lang['help_ftpserver'] ?>:</b> es la dirección del servidor de FTP destinatario del archivo (por ejemplo ftp.misbackups.com).
				</li>
				<li>
					<a name="conf15"></a><b><?php echo $lang['help_ftpport'] ?>:</b> es el puerto de conexión del servidor FTP (generalmente, el puerto 21).
				</li>
				<li>
					<a name="conf16"></a><b><?php echo $lang['help_ftpuser'] ?>:</b> es el nombre de usuario con el que debe realizarse la conexión.
				</li>
				<li>
					<a name="conf17"></a><b><?php echo $lang['help_ftppass'] ?>:</b> es el password a utilizar para establecer la conexión.
				</li>
				<li>
					<a name="conf18"></a><b><?php echo $lang['help_ftpdir'] ?>:</b> es el directorio de destino del archivo a almacenar. Puede ser un camino absoluto o relativo (pero debe tener derechos de escritura en el mismo).
				</li>
			</ul>
			<li>
				<b><?php echo $lang['config_cronperl'] ?></b> permite definir las características de la copia de seguridad utilizando el script Perl. La mayoría de las opciones son muy parecidas a las establecidas para el interfaz gráfico, pero para el script Perl, que actúa de forma independiente.
			</li>
		</ul>
