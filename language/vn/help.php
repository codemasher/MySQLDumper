<div id="content">
	<h3>Về dự án này</h3>
	Ý tưởng cho dự án này là của Daniel Schlichtholz.<p>Năm 2004, ông tạo ra diễn đàn
		<a href="http://forum.mysqldumper.de" target="_blank">MySQLDumper</a> và sau đó, lập trình những đoạn mã mới, bổ sung đoạn mã của Daniel.<br>Sau một thời gian ngắn, mã nguồn phát triển thành một dự án lớn hơn.
	<p>Nếu bạn có bất cứ góp ý nào nhằm cải tiến mã nguồn, bạn có thể truy cập vào Diễn đàn MySQLDumper:
		<a href="http://forum.mysqldumper.de" target="_blank">http://forum.mysqldumper.de</a>.
	<p>Chúc bạn có những giây phút thú vị.<br><br><h4>The MySQLDumper-Team</h4>
	<table>
		<tr>
	...
	<ul>
		...
		<ul>
			<li><a name="conf1"><strong>Cấu hình CSDL:</strong> Danh scahs của cấu hình CSDL. CSDL đang hoạt động được tô đậm.
			</li>
			<li>
				<a name="conf2"><strong>Table-Prefix (tiền tố bảng):</strong> bạn có thể chọn 1 tiền tố (prefix) cho mỗi CSDL. Tiền tố là một dạng lọc, nó chỉ thao tác với các bảng trong một khu vực, bắt đầu bằng tiền tố này (vd: tất cả các bagr bắt đầu bằng "phpBB_"). Nếu bạn không muốn sử dụng nó, hãy để trống trường này.
			</li>
			<li>
				<a name="conf3"><strong>Nén GZip:</strong> Dùng để kích hoạt chế độ nén. Hãy sử dụng nếu có thể vì nó giúp bạn nén nhỏ file, tiết kiệm dung lượng host, giảm thời gian download và do đó tiết kiệm băng thông.
			</li>
			<li>
				<a name="conf19"></a><strong>Số bản ghi khi Sao lưu:</strong> Đây là số của các bản ghi được đọc cùng lúc trong khi sao lưu, trước khi gọi các tập lệnh. Nếu nó làm chậm máy chủ, bạn có thể giảm tham số này để ngăn ngừa timeouts.
			</li>
			<li>
				<a name="conf20"></a><strong>Số bản ghi khi phục hồi:</strong> Đây là số của các bản ghi được đọc cùng lúc trong khi sao lưu, trước khi gọi các tập lệnh. Nếu nó làm chậm máy chủ, bạn có thể giảm tham số này để ngăn ngừa timeouts.
			</li>
			<li>
				<a name="conf4"></a><strong>Thư mục chứa file Backup:</strong> Chọn thư mục chứa file Backup. Nếu thư mục này chưa có, hệ thống sẽ tạo nó cho bạn. Có thể sử dụng đường dẫn tương đối hoặc tuyệt đối.
			</li>
			<li>
				<a name="conf5"></a><strong>Gửi file sao lưu vào email:</strong> Cho phép hệ thống gửi một email đính kèm file backup tới địa chỉ email được chỉ ra bên dưới (cẩn trọng khi sử dụng!, bạn phải chọn tùy chọn nén file khi sử dụng tính năng này vì file quá lớn có thể không gửi vào email được!)
			</li>
			...
			<li>
				<a name="conf13"></a><strong>FTP-Transfer: </strong>Tùy chọn này cho phép hệ thống tự động gửi file backup bằng phương thức FTP.
			</li>
			...
			<li><a name="conf18"></a><strong>FTP Upload-Ordner: </strong>thư mục chứa file backup (phải được cho phép upload lên!)
			</li>

			<li>
				<a name="conf8"></a><strong>Tự động xóa file backup:</strong> Tùy chọn này cho phép tự động xóa theo một quy luật được thiết lập trước.
			</li>
			<li>
				<a name="conf10"></a><strong>Xóa file nếu số lượng vượt quá:</strong> Nếu các file có số lương nhiều hơn giá trị được chỉ ra thì file cũ sẽ bị xóa.
			</li>
			<li>
				<a name="conf11"></a><strong>Language (ngôn ngữ):</strong> choose your language for the interface (chọn ngôn ngữ bạn muốn sử dụng cho chương trình này).
			</li>
		</ul>
		...
		<UL>
			...
			<li>
				<strong>Bắt đầu 1 sao lưu (Dump):</strong> tại đây bạn có thể bắt đầu 1 sao lưu (dump) mới với các thông số đã được cấu hình.
			</li>
		</UL>
