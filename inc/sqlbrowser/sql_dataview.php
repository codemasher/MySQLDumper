...
if($numrowsabs > 0 && $Anzahl_SQLs <= 1){
	...
	if($numrows > 0){
		...
		for($x = 0; $x < count($temp); $x++){
			...
			if(isset($str->Comment) && substr($str->Comment, 0, 4) == 'VIEW'){
			...
			if((isset($str->primary_key) && ($str->primary_key == 1)) || (isset($str->unique_key) && ($str->unique_key == 1))){
				...
				if($str->primary_key == 1){
					$pic = $icon['key_primary'];
				}
				elseif($str->unique_key == 1){
					$pic = $icon['index'];
				}
			}
			...
			if($bb == -1){
				...
				$bb_link = ($str->type == 'blob') ? '&nbsp;&nbsp;&nbsp;<a title="use BB-Code for this field" href="sql.php?db='.$db.'&amp;bb=-1&amp;tablename='.$tablename.'&amp;dbid='.$dbid.'&amp;order='.urlencode($order).'&amp;orderdir='.$orderdir.'&amp;limitstart='.$limitstart.'&amp;sql_statement='.urlencode($sql['sql_statement']).'&amp;tdc='.$tdcompact.'">[no BB]</a>' : '';
			}
