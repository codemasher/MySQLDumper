...
function GetMySQLVersion(){
	...
	$version = $row[0];
	if(!defined('MSD_MYSQL_VERSION')){
		define('MSD_MYSQL_VERSION', $version);
	}
	$versions = explode('.', $version);
	$new      = false;
	if($versions[0] == 4 && $versions[1] >= 1){
		$new = true;
	}
	if($versions[0] > 4){
		$new = true;
	}
	if(!defined('MSD_NEW_VERSION')){
		define('MSD_NEW_VERSION', $new);
	}
	...
	for($ii = 0; $ii < count($databases['multi']); $ii++){
		...
		if($num_tables > 0){
			for($i = 0; $i < $num_tables; $i++){
				...
				if(strtoupper($row['Comment']) == 'VIEW'
				   || (isset($row['Engine'])
				       && in_array(strtoupper($row['Engine']), [
							'MEMORY',
						]))){
