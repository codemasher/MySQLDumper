...
function get_sqlbefehl(){
	...
	while($sqlparser_status != 100 && !$restore['fileEOF'] && !$restore['EOB']){
		...
		// Index
		elseif($sqlparser_status == 4){ //Createindex
			if($last_char == ';'){
				if($config['minspeed'] > 0){
					$restore['anzahl_zeilen'] = $config['minspeed'];
				}
		...
		elseif($sqlparser_status == 5){ //Anweisung
			$t = strrpos($zeile, '*/;');
			if(!$t === false){
				$restore['anzahl_zeilen'] = $config['minspeed'];
				$sqlparser_status         = 100;
		...
		elseif($sqlparser_status == 7){ //Anweisung
			if($last_char == ';'){
				if($config['minspeed'] > 0){
					$restore['anzahl_zeilen'] = $config['minspeed'];
				}
