...
$msg  = '';
$dump = [];
if(isset($config['auto_delete']) && ($config['auto_delete'] == 1)){
	$msg = AutoDelete();
}
...
if(isset($_POST['dump_tbl'])){
	...
	echo '<script>parent.MySQL_Dumper_content.location.href="dump.php?comment='.urlencode($dk).'&sel_dump_encoding='.$dump['sel_dump_encoding'].'&config='.urlencode($config['config_file']).'";</script></body></html>';
	exit();
}
...
if(isset($_POST['restore_tbl'])){
	$databases['db_actual_tableselected'] = substr($_POST['tbl_array'], 0, strlen($_POST['tbl_array']) - 1);
	WriteParams();
	echo '<script>parent.MySQL_Dumper_content.location.href="restore.php?filename='.urlencode($_POST['filename']).'";</script></body></html>';
...
if(isset($_POST['restore'])){
	if(isset($_POST['file'])){
		if(isset($_POST['tblfrage']) && $_POST['tblfrage'] == 1){
			...
			$databases['db_actual_tableselected'] = '';
			WriteParams();
			echo '<script>parent.MySQL_Dumper_content.location.href="restore.php?filename='.$file.'&dump_encoding='.$dump_encoding.'&kind='.$kind.'";</script></body></html>';
			exit();
		}
	}
	else{
